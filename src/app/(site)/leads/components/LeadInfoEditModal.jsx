    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("lead:updated", {
          detail: { id: lead._id, fields: payload },
        })
      );
      ...
      if (statusChanged) {
        ...
        if (!jsonStatus.success)
          throw new Error(jsonStatus.error || "Gagal memperbarui status");
        latestDoc = jsonStatus.data || latestDoc;
        window.dispatchEvent(
          new CustomEvent("lead:status", {
            detail: { id: lead._id, status: form.status },
          })
        );
      }
      ...
      Swal.fire({
        icon: "success",
        title: "Tersimpan",
        text: "Informasi lead diperbarui",
        timer: 1600,
      ...
      Swal.fire({
        icon: "error",
        title: "Gagal",
        text: e2.message || "Tidak bisa menyimpan",
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      <form
        ...
        className="bg-white w-full sm:max-w-md rounded-t-xl sm:rounded-xl p-5 flex flex-col gap-4 animate-sheet-up sm:animate-scale-in"
      >
        ...
        <div className="grid gap-3">
          ...
          <Field
            ...
            disabled={
              form.source === "WhatsApp" || form.source === "Leads Kantor"
            }
          />
          <Field
            label="Email"
          ...
          <div className="flex flex-col gap-1">
            ...
            <PropertyTypeahead
              ...
              onInput={(val) =>
                setForm((f) => ({ ...f, propertyName: val, property: null }))
              }
              onSelect={(it) =>
                setForm((f) => ({
                  ...f,
            ...
            />
            {form.property && (
              <p className="text-[10px] text-green-600">Terpilih dari master</p>
            )}
          </div>
          <Field
            label="Unit"
          ...
          <div className="flex flex-col gap-1 relative" ref={sourceRef}>
            ...
            <button
              ...
              className="relative rounded border border-slate-300 px-3 py-2 text-sm bg-white focus:ring-2 focus:ring-blue-400 outline-none flex justify-between items-center"
            >
              <span className="truncate text-left">
                {form.source || "Pilih sumber"}
              </span>
              <span
                className={`transition text-[10px] ${
                ...
                {SOURCES.map((s) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 hover:bg-blue-50 ${
                      form.source === s ? "bg-blue-100" : ""
                    } ${form.source === "WhatsApp" || form.source === "Leads Kantor" ? "opacity-50 cursor-not-allowed" : ""}`}
                    disabled={
                      form.source === "WhatsApp" ||
                      form.source === "Leads Kantor"
                    }
                  >
                    {s}
                  </button>
                  ...
                  {STATUSES.map((s) => (
                    <li
                      ...
                      className={`px-3 py-2 text-sm cursor-pointer hover:bg-blue-50 ${
                        form.status === s ? "bg-blue-50 font-medium" : ""
                      }`}
                    >
