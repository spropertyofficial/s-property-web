// src/app/(site)/properties/[type]/[id]/page.js
          ...
          {property.description && (
            <section className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="text-gray-600 leading-relaxed">
                {parse(marked(property.description))}
              </div>
            </section>
          )}
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    description: `${plainProperty.name} di ${plainProperty.location?.area}, ${plainProperty.location?.city}. Mulai dari ${plainProperty.startPrice ? `Rp ${plainProperty.startPrice.toLocaleString("id-ID")}` : "Hubungi kami untuk harga"}.`,
    openGraph: {
      title: `${plainProperty.name} - S-Property`,
      description: `${plainProperty.name} di ${plainProperty.location?.area}, ${plainProperty.location?.city}`,
      images:
        plainProperty.gallery && plainProperty.gallery.length > 0
          ? [plainProperty.gallery[0].src]
          : [],
    },
  };
}
