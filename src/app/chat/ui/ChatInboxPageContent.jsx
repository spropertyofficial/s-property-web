...
export default function ChatInboxPageContent() {
  ...
  const [selectedId, setSelectedId] = useState(conversations[0]?.lead?._id || null);
  const [search, setSearch] = useState("");
  const [filter, setFilter] = useState("all");
  const [showInfo, setShowInfo] = useState(false);

  // Derived: selected conversation and its messages
  const selected = useMemo(() => conversations.find(c => c.lead?._id === selectedId) || null, [conversations, selectedId]);
  const messages = useMemo(() => (selected ? (messagesById[selected.lead._id] || []) : []), [messagesById, selected]);
  ...
  if (isLoading) return <div className="p-8 text-center">Loading percakapan...</div>;
  if (error) return <div className="p-8 text-center text-red-500">Gagal memuat data: {error.message}</div>;
