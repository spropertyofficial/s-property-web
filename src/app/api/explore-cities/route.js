...
export async function GET(req) {
  try {
    ...
    const images = await RegionCityImage.find({
      isActive: true
    }).select("name type slug image").lean();
      ...
      if (region && city) {
        ...
        if (!data[region]) {
          ...
          data[region] = {
            ...
            imageUrl: imageMap[regionKey] || `/images/Regions/${region
              .toLowerCase()
              .replace(/\s+/g, "-")}.webp`,
            cities: {},
          };
        ...
        if (!data[region].cities[city]) {
          ...
          data[region].cities[city] = {
            ...
            propertyCount: 0,
            // Cek apakah ada gambar di CMS, jika tidak gunakan fallback
            imageUrl: imageMap[cityKey] || `/images/Cities/${city
              .toLowerCase()
              .replace(/\s+/g, "-")}.webp`,
          };
        }
    ...
    return NextResponse.json({ 
      ...
      meta: {
        ...
        totalCities: Object.values(data).reduce((acc, region) => 
          acc + Object.keys(region.cities).length, 0
        ),
