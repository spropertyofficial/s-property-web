...
export default function AdminRukoPage() {
  const [shophouses, setShophouses] = useState([]);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/properties?assetType=Ruko");
      if (!res.ok) {
        throw new Error("Gagal mengambil data dari server");
      }
      const data = await res.json();
      console.log("Fetched shophouses:", data);
      setShophouses(data.properties|| []);
    } catch (error) {
  ...
  const filteredData = shophouses.filter((r) => {
    const matchesSearch =
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-500">
                {shophouses.filter((r) => r.propertyStatus === "SALE").length}
              </h3>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-500">
                {shophouses.filter((r) => r.propertyStatus === "RENT").length}
              </h3>
            </div>
          </div>
        </div>
